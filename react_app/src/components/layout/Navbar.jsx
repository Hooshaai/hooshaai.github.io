  ...
  return (
    <header className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/95 backdrop-blur-2xl border-b border-gray-800 shadow-[0_10px_30px_rgba(0,0,0,0.9)] py-3' : 'bg-gradient-to-b from-black/90 via-black/50 to-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center gap-4">
        ...
        {/* Brand Header */}
        <Link to="/" className="flex items-center gap-3.5 group no-underline text-left">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 bg-gray-950/80 p-2 rounded-full border border-gray-800/90 backdrop-blur-2xl shadow-2xl">
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Cluster Status Pill */}
          <div className="hidden sm:flex items-center gap-2 px-3.5 py-1.5 bg-gray-900/90 border border-gray-800 rounded-full text-xs font-mono text-gray-300 shadow-inner">
          ...
          <button
            onClick={toggleSound}
            aria-label="Toggle audio synth effects"
          ...
          <div className="relative">
            <button
              ...
              className="w-9 h-9 rounded-full bg-gray-900/90 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-white transition-colors relative"
              title="Notifications"
            >
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-2 bg-gray-900/90 hover:bg-gray-800 text-gray-300 px-3.5 py-1.5 rounded-full border border-gray-800 text-xs font-mono transition-colors shadow-inner"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="px-6 py-6 space-y-3 text-left">
              {navItems.map((item) => (
                <NavLink
                  key={item.path}
                  to={item.path}
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 p-3.5 rounded-xl text-sm font-mono transition-colors ${
                      isActive
                        ? item.danger
              ...
              <div className="pt-4 border-t border-gray-800 flex justify-between items-center">
                <button
                  ...
                  className="flex-1 py-3 bg-black border border-gray-800 rounded-xl text-xs font-mono text-cyan-400 flex items-center justify-center gap-2"
                >

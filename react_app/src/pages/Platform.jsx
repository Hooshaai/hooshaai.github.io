...
const Platform = () => {
    ...
    const id = setInterval(() => {
      ...
      setChartData(prev => {
        ...
        newData.push({
          ...
          eval: Math.max(0.1, lastLoss - Math.random() * 0.01 + 0.1)
        });
        return newData;
      });
    }, 1000);
  ...
  const handleCompile = () => {
    setCompiling(true);
    setCompilerOutput('Compiling Triton kernel to PTX...\n');
    setTimeout(() => setCompilerOutput(prev => prev + 'Optimizing memory access patterns...\n'), 800);
    setTimeout(() => setCompilerOutput(prev => prev + 'Allocating shared memory (48KB)...\n'), 1500);
    setTimeout(() => {
      setCompilerOutput(prev => prev + '\n✓ Kernel compilation successful. Launching on H100 array...\nAchieved 87% Peak TFLOPS.');
      setCompiling(false);
    }, 2500);
  };
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-2xl p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {telemetry.map(gpu => (
                <div key={gpu.id} className="bg-black/60 border border-gray-800 rounded-xl p-4 flex flex-col gap-3 relative overflow-hidden group hover:border-cyan-500/30 transition-colors">
                  ...
                  <div className="text-xs font-bold font-mono text-gray-300 flex justify-between mt-2">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-gray-500 font-mono"><span>VRAM</span> <span>{gpu.vram.toFixed(1)} / 80 GB</span></div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-gray-500 font-mono"><span>Mem BW</span> <span>{gpu.memory_bw.toFixed(0)} GB/s</span></div>
                  </div>
                  
                  <div className="text-right text-xl font-bold font-mono text-white mt-auto pt-2 border-t border-gray-800">
                    {gpu.usage.toFixed(0)}<span className="text-xs text-gray-500 ml-1">% Util</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-2xl p-6 shadow-xl">
             <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold font-['Space_Grotesk'] flex items-center"><i className="fas fa-chart-line text-purple-400 mr-3"></i>Training Loss (CFM-7B-v2)</h2>
              <div className="flex gap-4 text-xs font-mono">
                <span className="flex items-center"><span className="w-3 h-3 bg-cyan-400 rounded-full mr-2"></span>Train Loss</span>
                <span className="flex items-center"><span className="w-3 h-3 bg-purple-500 rounded-full mr-2"></span>Eval Loss</span>
              </div>
            </div>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  ...
                  <XAxis dataKey="step" stroke="#9ca3af" tick={{fontSize: 12, fill: '#9ca3af', fontFamily: 'monospace'}} />
                  <YAxis stroke="#9ca3af" tick={{fontSize: 12, fill: '#9ca3af', fontFamily: 'monospace'}} domain={[0, 'auto']} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '8px', fontFamily: 'monospace' }}
                    itemStyle={{ color: '#fff' }}
                  />
                  <Line type="monotone" dataKey="loss" stroke="#22d3ee" strokeWidth={2} dot={false} isAnimationActive={false} />
                  <Line type="monotone" dataKey="eval" stroke="#a855f7" strokeWidth={2} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-2xl p-6 shadow-xl h-full flex flex-col">
            <h2 className="text-xl font-bold mb-4 font-['Space_Grotesk'] flex items-center"><i className="fas fa-code text-green-400 mr-3"></i>Kernel Playground</h2>
            <div className="bg-black border border-gray-700 rounded-xl overflow-hidden font-mono text-sm flex-1 flex flex-col">
              <div className="bg-gray-800 px-4 py-3 flex justify-between items-center text-gray-300 border-b border-gray-700">
                <span className="flex items-center gap-2"><i className="fab fa-python text-yellow-400"></i> flash_attn.py</span>
                <button 
                ...
                >
                  {compiling ? <><i className="fas fa-spinner fa-spin mr-2"></i>Compiling...</> : <><i className="fas fa-play mr-2"></i>Run</>}
                </button>
              </div>
              <textarea 
                className="w-full h-48 bg-transparent p-4 text-gray-300 focus:outline-none resize-none leading-relaxed"
                spellCheck="false"
                defaultValue={`@triton.jit\\ndef flash_attn_kernel(\\n    Q, K, V, Out,\\n    sm_scale,\\n    BLOCK_M: tl.constexpr,\\n    BLOCK_N: tl.constexpr\\n):\\n    offs_m = tl.arange(0, BLOCK_M)\\n    offs_n = tl.arange(0, BLOCK_N)\\n    # Load blocks\\n    q = tl.load(Q_block_ptr)\\n    k = tl.load(K_block_ptr)`}
              />
              <div className="bg-gray-950 p-4 border-t border-gray-800 h-32 overflow-y-auto">
                <div className="text-xs text-gray-500 mb-2 uppercase tracking-wider font-bold">Compiler Output</div>
                <pre className="text-green-400 text-xs whitespace-pre-wrap">{compilerOutput || 'Ready.'}</pre>
              </div>
            </div>
          </div>
